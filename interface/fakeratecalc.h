...
#ifndef fakeratecalc_h
#define fakeratecalc_h
...
public :
   ...
   fakeratecalc(TTree *tree=0);
   fakeratecalc(DMCblock* block);
   fakeratecalc(string blockname);
   virtual ~fakeratecalc();
   //virtual Int_t    Cut(Long64_t entry);
   virtual Int_t    GetEntry(Long64_t entry);
   virtual Long64_t LoadTree(Long64_t entry);
   virtual void     Init(TTree *tree);
   virtual void     Loop(eventRegistry* EventRegistry,eventRegistry* BadEventRegistry);
...
#ifdef fakeratecalc_cxx
fakeratecalc::fakeratecalc(TTree *tree) : fChain(0) 
{
...
// used to generate this class and read the Tree.
   ...
   Init(tree);
}//end simple fakeratecalc

fakeratecalc::fakeratecalc(DMCblock* block){
...
}//end fakeratecalc for DMCblock

fakeratecalc::fakeratecalc(string blockname) : fakeratecalc(setupDMCblock(blockname,"")){
}

fakeratecalc::~fakeratecalc()
{
   if (!fChain) return;
   delete fChain->GetCurrentFile();
}

Int_t fakeratecalc::GetEntry(Long64_t entry)
{
// Read contents of entry.
   if (!fChain) return 0;
   return fChain->GetEntry(entry);
}
Long64_t fakeratecalc::LoadTree(Long64_t entry)
{
...
void fakeratecalc::Init(TTree *tree)
{
...
void fakeratecalc::Show(Long64_t entry)
{
...
/*Int_t fakeratecalc::Cut(Long64_t entry)
{
...
void fakeratecalc::IncludeAJson(std::string jsonfile) {
...
bool fakeratecalc::isInJson(Int_t run,Int_t lumi) {
	return goodrunlumilist[run][lumi];
}

#endif // #ifdef fakeratecalc_cxx
